...
import { Attachment, chatSessionsAPI } from '@/lib/api';
...
function statusBadge(status: string) {
  if (status === 'completed')
    return <span className="text-green-700 text-xs font-semibold">✓ Transcribed</span>;
  if (status === 'pending')
    return <span className="text-amber-700 text-xs font-semibold">⏳ Transcribing…</span>;
  return <span className="text-red-700 text-xs font-semibold">✗ Failed</span>;
}

export default function TicketAttachments({ ticketId, attachments }: Props) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-100">
      <h3 className="font-bold text-lg mb-4">📎 Evidence attachments ({attachments.length})</h3>
      <ul className="space-y-3">
        {attachments.map(att => {
          const isImage = att.mimeType.startsWith('image/');
          const downloadUrl = chatSessionsAPI.ticketAttachmentUrl(ticketId, att.id);
